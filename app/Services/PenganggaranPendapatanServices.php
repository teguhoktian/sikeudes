...
class PenganggaranPendapatanServices
{
    ...
    public function getDataTables($tahunAnggaran)
    {
        ...
        return Datatables::of($data)
            ...
            ->addColumn('action', function ($row) use ($tahunAnggaran) {
                $button = '';
                $button .= '<a href="' . route('penganggaran.pendapatan.detail.index', ['tahun_anggaran' => $tahunAnggaran->id, 'rekening_objek' => $row->id_rekening_objek]) . '" class="btn btn-sm btn-success btn-rounded"><i class=" mdi mdi-plus-circle-outline"></i> ' . __('Detail') . '</a>';
                return $button;
            })
    ...
    {
        $query = PenganggaranPendapatan::selectRaw('penganggaran_pendapatan.*, (harga_satuan * volume) AS total')
            ->where([
        ...
        return Datatables::of($query)
            ...
            ->addColumn('action', function ($row) {
                $button = '';
                $button .= '<a href="' . route('penganggaran.pendapatan.edit', ['pendapatan' => $row->id]) . '" class="btn btn-sm btn-success btn-rounded"><i class=" mdi mdi-lead-pencil"></i> ' . __('Edit') . '</a>';
                return $button;
            })
