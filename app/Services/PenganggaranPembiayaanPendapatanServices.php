...
class PenganggaranPembiayaanPendapatanServices
{
    ...
    public function getDataTables($tahunAnggaran)
    {
        ...
        return Datatables::of($data)
            ...
            ->addColumn('action', function ($row) use ($tahunAnggaran) {
                $button = '';
                $button .= '<a href="' . route('penganggaran.biaya-pendapatan.detail.index', ['tahun_anggaran' => $tahunAnggaran->id, 'rekening_objek' => $row->id_rekening_objek]) . '" class="btn btn-sm btn-success btn-rounded"><i class=" mdi mdi-plus-circle-outline"></i> ' . __('Detail') . '</a>';
                return $button;
            })
    ...
    {
        $query = PenganggaranPembiayaanPendapatan::selectRaw('penganggaran_pembiayaan_pendapatan.*, (harga_satuan * volume) AS total')
            ->where([
        ...
        return Datatables::of($query)
            ...
            ->addColumn('action', function ($row) {
                $button = '';
                $button .= '<a href="' . route('penganggaran.biaya-pendapatan.edit', ['biaya_pendapatan' => $row->id]) . '" class="btn btn-sm btn-success btn-rounded"><i class=" mdi mdi-lead-pencil"></i> ' . __('Edit') . '</a>';
                return $button;
            })
    ...
    public function store($tahun, $request)
    {
        ...
        return PenganggaranPembiayaanPendapatan::create($request->all());
    }
    ...
    public function pendapatanOptions()
    {
        return \App\Models\RekeningObjek::selectRaw('CONCAT(rekening_akun.kode,".",rekening_kelompok.kode,".", rekening_jenis.kode, ".", rekening_objek.kode, " ", rekening_objek.nama) AS nama, rekening_objek.id')
            ...
            ->where(['rekening_akun.kode' => '6', 'rekening_kelompok.kode' => '1'])
