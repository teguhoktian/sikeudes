...
<div class="container">
    <div class="row">
        <div class="col-xs-12">
            <div class="page-title-box">
                <h4 class="page-title">{{ __('RPJMD Sub Bidang') }}</h4>
                <ol class="breadcrumb p-0 m-0">
                    <li>
                        <a href="/">{{ __('Beranda') }}</a>
                    </li>
                    <li>
                        <a href="/">{{ __('RPJMD Sub Bidang') }} </a>
                    </li>
                    <li class="active">
                        {{ __('RPJMD Sub Bidang') }}
                    </li>
                    ...
                    <table id="datatable" class="table table-striped table-bordered table-colored-bordered table-bordered-brown dt-responsive nowrap">
                        <thead>
                            <tr>
                                ...
                                <th colspan="2">
                                    {{ $bidang->visi->desa->kode }}.{{ $bidang->visi->kode }}
                                    - {{ $bidang->visi->uraian }}
                                </th>
                            </tr>
                            <tr>
                                <th colspan="2">Bidang</th>
                                <th colspan="2">
                                    {{ $bidang->visi->desa->kode }}.{{ $bidang->visi->kode }}.{{ $bidang->bidang->kode }}
                                    - {{ $bidang->bidang->nama }}
                                </th>
                            </tr>
                            <tr>
    ...
    <div class="row">
        {!! Form::open(['route' => ['rpjmd.subbidang.destroy', ['bidang' => $bidang->id]], 'id' => 'deleteForm']) !!}
        </form>
    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        var routeAdd = '{{route("rpjmd.subbidang.create", ["bidang" => $bidang->id])}}';
        var routeUp = '{{ route("rpjmd.bidang.index", ["visi" => $bidang->visi->id]) }}';
        var IndexRoute = '{{ route("rpjmd.subbidang.index", ["bidang" => $bidang->id]) }}';
