    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="page-title-box">
                <h4 class="page-title">{{ __('Detail Penganggaran Pembiayaan Pendapatan') }} - {{ Auth::user()->desa->nama }}</h4>
                <ol class="breadcrumb p-0 m-0">
                    <li>
                        <a href="/">{{ __('Beranda') }}</a>
                    </li>
                    <li>
                        <a href="/">{{ __('Penganggaran Pembiayaan Pendapatan') }} </a>
                    </li>
                    <li class="active">
                        {{ __('Detail Penganggaran Pembiayaan Pendapatan') }}
                    </li>
                </ol>
...
<div class="row">
    {!! Form::open(['route' => ['penganggaran.biaya-pendapatan.destroy', ['tahun_anggaran' => $tahunAnggaran->id, 'rekening_objek' => $rekeningObjek->id]], 'id' => 'deleteForm']) !!}
    </form>
</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        var routeAdd = '{{route("penganggaran.biaya-pendapatan.create", ["tahun_anggaran" => $tahunAnggaran->id])}}';
        var IndexRoute = '{{ route("penganggaran.biaya-pendapatan.detail.index", ["tahun_anggaran" => $tahunAnggaran->id, "rekening_objek" => $rekeningObjek->id]) }}';
        var textAdd = '{{ __("Tambah Data") }}';
        var textDelete = '<i class="mdi mdi-delete-forever"></i> {{ __("Hapus") }}';
        var routeUp = '{{ route("penganggaran.biaya-pendapatan.index", ["tahun_anggaran" => $tahunAnggaran->id])}}';
