    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="page-title-box">
                <h4 class="page-title">{{ __('Penganggaran Biaya Pendapatan') }} - {{ Auth::user()->desa->nama }}</h4>
                <ol class="breadcrumb p-0 m-0">
                    <li>
                        <a href="/">{{ __('Beranda') }}</a>
                    </li>
                    <li>
                        <a href="/">{{ __('Penganggaran Biaya Pendapatan') }} </a>
                    </li>
                    <li class="active">
                        {{ __('Penganggaran Biaya Pendapatan') }}
                    </li>
                </ol>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var IndexRoute = '{{ route("penganggaran.biaya-pendapatan.index", ["tahun_anggaran" => $tahunAnggaran->id]) }}';
        var textAdd = '{{ __("Tambah Data") }}';
        var textDelete = '<i class="mdi mdi-delete-forever"></i> {{ __("Hapus") }}';
        var routeUp = '{{ route("biaya-pendapatan.tahun-anggaran.index")}}';
