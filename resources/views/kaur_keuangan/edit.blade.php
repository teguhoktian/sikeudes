    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="page-title-box">
                <h4 class="page-title">{{ __('Kaur Keuangan') }} - {{ Auth::user()->desa->nama }}</h4>
                <ol class="breadcrumb p-0 m-0">
                    <li>
                        <a href="/">{{ __('Beranda') }}</a>
                    </li>
                    <li>
                        <a href="/">{{ __('Kaur Keuangan') }} </a>
                    </li>
                    <li class="active">
                        {{ __('Kaur Keuangan') }}
                    </li>
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card-box">
                ...
                {{ Form::model($kaurKeuangan, [ 'route' => ['kaur-keuangan.update', $kaurKeuangan->id], 'autocomplete' => 'off', 'method' => 'PATCH', 'enctype' => 'multipart/form-data' ]) }}

                @include('kaur_keuangan.form')
                <button class="btn btn-primary" type="submit"><i class="mdi mdi-content-save"></i> {{ __('Simpan') }}</button>
                <a href="{{ route('kaur-keuangan.index') }}" class="btn-danger btn">{{ __('Kembali') }}</a>
                </form>
            </div>
        </div>
    </div>
