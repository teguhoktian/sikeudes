        ...
        <div class="col-sm-12">
            <div class="card-box">
                ...
                <p class="text-muted m-b-30 font-13">
                    {{ __('Tambah Data Sub Bidang') }}
                </p>

                <div class="row">
                    <div class="col-md-12">
                        {!! Form::open(['route' => ['sub-bidang.store', 'bidang' => $bidang->id], 'autocomplete' => 'off', 'enctype' => 'multipart/form-data']) !!}
                        @include('sub_bidang.form')
                        <button class="btn btn-primary" type="submit"><i class="mdi mdi-content-save"></i> {{ __('Simpan') }}</button>
                        <a href="{{ route('sub-bidang.index', ['bidang' => $bidang->id]) }}" class="btn-danger btn">{{ __('Kembali') }}</a>
                        </form>
                    </div>
                </div>
            </div>
