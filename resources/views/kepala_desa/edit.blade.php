    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card-box">
                ...
                {{ Form::model($kepalaDesa, [ 'route' => ['kepala-desa.update', $kepalaDesa->id], 'autocomplete' => 'off', 'method' => 'PATCH', 'enctype' => 'multipart/form-data' ]) }}

                @include('kepala_desa.form')
                <button class="btn btn-primary" type="submit"><i class="mdi mdi-content-save"></i> {{ __('Simpan') }}</button>
                <a href="{{ route('kepala-desa.index') }}" class="btn-danger btn">{{ __('Kembali') }}</a>
                </form>
            </div>
        </div>
    </div>
