    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="page-title-box">
                <h4 class="page-title">{{ __('Sekretaris Desa') }} - {{ Auth::user()->desa->nama }}</h4>
                <ol class="breadcrumb p-0 m-0">
                    <li>
                        <a href="/">{{ __('Beranda') }}</a>
                    </li>
                    <li>
                        <a href="/">{{ __('Sekretaris Desa') }} </a>
                    </li>
                    <li class="active">
                        {{ __('Sekretaris Desa') }}
                    </li>
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card-box">
                ...
                {{ Form::model($sekretarisDesa, [ 'route' => ['sekretaris-desa.update', $sekretarisDesa->id], 'autocomplete' => 'off', 'method' => 'PATCH', 'enctype' => 'multipart/form-data' ]) }}

                @include('sekretaris_desa.form')
                <button class="btn btn-primary" type="submit"><i class="mdi mdi-content-save"></i> {{ __('Simpan') }}</button>
                <a href="{{ route('sekretaris-desa.index') }}" class="btn-danger btn">{{ __('Kembali') }}</a>
                </form>
            </div>
        </div>
    </div>
