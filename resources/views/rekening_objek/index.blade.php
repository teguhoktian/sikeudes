...
<div class="container">
        ...
        <div class="col-sm-12">
            <div class="card-box">
                <table class="table table-bordered">
                    <tr>
                        <th width="10%" colspan="3">Kode Akun</th>
                        <th>Nama Akun</th>
                    </tr>
                    <tr>
                        <td>{{ $rekeningJenis->kelompok->akun->kode }}</td>
                        <td></td>
                        <td></td>
                        <td>{{ $rekeningJenis->kelompok->akun->nama }}</td>
                    </tr>
                    <tr>
                        <td>{{ $rekeningJenis->kelompok->akun->kode }}</td>
                        <td>{{ $rekeningJenis->kelompok->kode }}</td>
                        <td></td>
                        <td>{{ $rekeningJenis->kelompok->nama }}</td>
                    </tr>
                    <tr>
                        <td>{{ $rekeningJenis->kelompok->akun->kode }}</td>
                        <td>{{ $rekeningJenis->kelompok->kode }}</td>
                        <td>{{ $rekeningJenis->kode }}</td>
                        <td>{{ $rekeningJenis->nama }}</td>
                    </tr>
                </table>
    ...
    <div class="row">
        {!! Form::open(['route' => ['rekening-objek.destroy', ['rekening_jenis' => $rekeningJenis->id]], 'id' => 'deleteForm']) !!}
        </form>
    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        var routeAdd = '{{route("rekening-objek.create", ["rekening_jenis" => $rekeningJenis->id])}}';
        var IndexRoute = '{{ route("rekening-objek.index", ["rekening_jenis" => $rekeningJenis->id]) }}';
        var routeUp = '{{ route("rekening-jenis.index", ["rekening_kelompok" => $rekeningJenis->kelompok->id]) }}';
