...
<div class="container">
        ...
        <div class="col-sm-12">
            <div class="card-box">
                <table class="table table-striped table-bordered">
                    <tr>
                        <th width="10%">{{ __('Kode Visi') }}</th>
                        <th>{{ __('Uraian') }}</th>
                    </tr>
                    <tr>
                        <td width="10%">
                            {{ Auth::user()->desa->kecamatan->kode }}.{{ Auth::user()->desa->kode }}.{{ $visi->kode }}
                        </td>
                        <td>{{ $visi->uraian }}</td>
                    </tr>
                </table>
    ...
    <div class="row">
        {!! Form::open(['route' => ['misi.destroy', ['visi' => $visi->id]], 'id' => 'deleteForm']) !!}
        </form>
    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        var routeAdd = '{{route("misi.create", ["visi" => $visi->id])}}';
        var IndexRoute = '{{ route("misi.index", ["visi" => $visi->id]) }}';
        var routeUp = '{{ route("visi.index") }}';
