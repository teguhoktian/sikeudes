    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="page-title-box">
                <h4 class="page-title">{{ __('RPJMD Kegiatan') }} - {{ Auth::user()->desa->nama }}</h4>
                <ol class="breadcrumb p-0 m-0">
                    <li>
                        <a href="/">{{ __('Beranda') }}</a>
                    </li>
                    <li>
                        <a href="/">{{ __('RPJMD Kegiatan') }} </a>
                    </li>
                    <li class="active">
                        {{ __('RPJMD Kegiatan') }}
                    </li>
                ...
                <table id="datatable" class="table table-striped table-bordered nowrap table-colored-bordered table-bordered-brown">
                    <thead>
                        <tr>
                            ...
                            <th colspan="2">
                                {{ $subBidang->rpjmd_bidang->visi->desa->kode }}.{{ $subBidang->rpjmd_bidang->visi->kode }}
                                - {{ $subBidang->rpjmd_bidang->visi->uraian }}
                            </th>
                        </tr>
                        <tr>
                            <th colspan="2"> {{ __('Bidang') }}</th>
                            <th colspan="2">
                                {{ $subBidang->rpjmd_bidang->visi->desa->kode }}.{{ $subBidang->rpjmd_bidang->visi->kode }}.{{ $subBidang->sub_bidang->bidang->kode }}
                        ...
                        <tr>
                            ...
                            <th colspan="2">
                                {{ $subBidang->rpjmd_bidang->visi->desa->kode }}.{{ $subBidang->rpjmd_bidang->visi->kode }}.{{ $subBidang->sub_bidang->bidang->kode }}.{{ $subBidang->sub_bidang->kode }}
...
<div class="row">
    {!! Form::open(['route' => ['rpjmd.kegiatan.destroy', ['sub_bidang' => $subBidang->id]], 'id' => 'deleteForm']) !!}
    </form>
</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        var routeAdd = '{{route("rpjmd.kegiatan.create", ["sub_bidang" => $subBidang->id])}}';
        var IndexRoute = '{{ route("rpjmd.kegiatan.index", ["sub_bidang" => $subBidang->id]) }}';
        var textAdd = '{{ __("Tambah Data") }}';
        var textDelete = '<i class="mdi mdi-delete-forever"></i> {{ __("Hapus") }}';
        var routeUp = '{{ route("rpjmd.subbidang.index", ["bidang" => $subBidang->rpjmd_bidang->id]) }}';
